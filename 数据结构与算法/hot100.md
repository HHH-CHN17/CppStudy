...
public:
    int lengthOfLongestSubstring(string s) {
        ...
        int res = 0;
        int currlen = 0;
        unordered_map<char, int> um_has_read{};
        for (right = 0; right < s.size(); right++) {
            auto it = um_has_read.find(s[right]);
            if (it != um_has_read.end() && it->second >= left) {  // 当前字串有重复
                currlen = right - left;
                res = max(res, currlen);
                left = it->second + 1;
                // insert()时如果有重复，则不会替换。
                //um_has_read.insert(make_pair(s[right], right));

                // 使用operator[]时，如果有重复，会发生替换
                um_has_read[s[right]] = right;
            }
            else {  // 当前字串无重复
                //um_has_read.insert(make_pair(s[right], right));
                um_has_read[s[right]] = right;
...
public:
    int maxSubArray(vector<int>& nums) {
        ...
        vector<int> subsum{};
        vector<int> currminsum{};
        int currsum = 0;
        int minsum = 0;
        subsum.push_back(0);
        currminsum.push_back(0);
        for (int i = 0; i < nums.size(); i++) {
            currsum += nums[i];
            subsum.push_back(currsum);
            
            minsum = min(currsum, minsum);
            currminsum.push_back(minsum);
        }

        int res = nums[0];
        for (int i = 0; i < nums.size(); i++) {
            int curr_max_sum = subsum[i + 1] - currminsum[i];
