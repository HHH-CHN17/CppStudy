...
public:
    int getMAxValue(vector<int>& space, vector<int>& value, int bagSpace) {
        vector<int> dp(bagSpace + 1);   // dp[j]：将物品逐个塞入背包中，当背包空间为j时，所能装下的最大价值为dp[j]
...
public:
    int getMAxValue(vector<int>& space, vector<int>& value, int bagSpace) {
        vector<int> dp(bagSpace + 1, 0);   // dp[j]：将物品逐个塞入背包中，当背包空间为j时，所能装下的最大价值为dp[j]
        int obj_num = space.size();
        for (int i = 0; i < dp.size(); i++) {
            if (i >= space[0])
...
public:
    int numSquares(int n) {
        vector<int> dp(n + 1);  // dp[i]：和为整数i的完全平方数的最少数量
        dp[0] = 0;
        for (int i = 1; i <= n; i++) {
            int sqrt_i = pow((int)sqrt(i), 2);
            dp[i] = 1 + dp[i - sqrt_i];
...
public:
    int coinChange(vector<int>& coins, int amount) {
        // dp[i][j]：从下标[0, i]的范围内选取硬币面额，使其总面额正好为j，最少的硬币个数为dp[i][j]
        ...
        for (int i = 1; i < coins.size(); i++) {
            for (int j = 1; j <= amount; j++) {
                ...
                if (expect_amount >= 0) {
                    dp[i][j] = min(dp[i - 1][j], dp[i][expect_amount] + 1);
                }
                else
                    dp[i][j] = dp[i - 1][j];
            }
        }
        return dp[coins.size() - 1][amount] == INT_MAX - 1 ? -1 : dp[coins.size() - 1][amount];
    }
};
```
