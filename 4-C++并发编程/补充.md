...
[(72 封私信 / 84 条消息) C++异常安全 - 知乎](https://zhuanlan.zhihu.com/p/318741315)
...
###### 1.2.1 异常安全的四个等级（从低到高）

- 等级 0：无保证 (No Guarantee)

  这是最糟糕的情况。如果异常发生了：

  - 内存可能泄漏。
  - 数据结构可能损坏（例如：`vector` 的 size 增加了，但元素没放进去）。
  - 程序处于未定义状态，甚至崩溃。
  - **例子**：C 语言风格的手动 `malloc/free` 混杂逻辑，中间出错了没 `free`。

- 等级 1：基本保证 (Basic Guarantee) —— “不泄露，不崩溃”

  这是 C++ 代码的**最低及格线**。如果异常发生了：

  - **没有资源泄漏**（所有申请的内存都会释放）。
  - **程序处于合法状态**，但**不确定是哪个状态**。
  - **例子**：你修改一个 `string` 失败了，原来的字符串内容可能变了，可能变空了，但这个 `string` 对象依然可以被安全地析构或重新赋值，不会导致程序崩掉。

- 等级 2：强保证 (Strong Guarantee) —— “事务性语义 (Commit or Rollback)”

  这是最理想的业务逻辑状态。如果异常发生了：

  - **程序状态回滚到操作调用之前**。就像数据库事务一样：要么全部成功，要么完全不发生。
  - **例子**：`std::vector::push_back`。如果扩容失败，你的 `vector` 会保持原样，没有任何数据丢失。

- 等级 3：不抛出保证 (Nothrow / No-fail Guarantee)

  这是最高等级，也是某些操作必须具备的特质。

  - 函数承诺**绝不抛出异常**。
  - **必须具备此保证的地方**：
    1. **析构函数**（`destructor`）：如果析构抛出异常，会导致程序直接终止（`std::terminate`），或者在栈展开（stack unwinding）时导致未定义行为。
    2. **移动构造/赋值**（`move constructor/assignment`）：如果移动操作抛出异常，会导致容器（如 `vector`）在扩容时陷入进退两难的境地。
    3. **内存释放函数**（`delete`, `free`）。
...
2.  **出栈**：只能从 `thread_local` 栈中弹出当前栈顶层级值对应的锁，否则unlock失败（**最好使用RAII进行管理，注意RAII类析构时需要pop栈和unlock，为避免栈弹出顺序不对，最好检查pop的锁和unlock的锁是不是为同一个**）。
