...
- `join()`后又 `detach()`
  ...
  `join()`本来就是等待线程执行结束的，线程都执行结束了，就没必要 `detach()`了
- `detach()`后又 `join()`

  `detach()`后子线程和主线程中的线程对象都分离了，再来 `join()` 没有意义
...
- 懒汉式
  ...
  - 在本例中，我们把`Destory()`函数设置为了private，但如果我们在程序中希望可以手动调用`Destory()`函数，为防止程序结束时重复释放，我们可以稍微改下`Destory()`：
    ...
    如上，但该函数仅能在单线程下使用，多线程需要使用原子变量结合cas进行操作，不过最好的办法还是直接隐藏，不给`Destory()`，如果我们有什么事情需要重启单例，可以运用移动构造进行重新赋值。
  
  - 当然我们还可以更进一步：在init时判断子类对应的构造函数和析构函数可以访问，这个需要通过static_assert实现，具体实现见泛型编程？？？（可以使用`is_default_constructible_v`和`is_destructible`等判断是否有默认构造函数，不能使用其他方法，具体原因我也不知道，但是用`is_default_constructible_v`和`is_destructible`要求子类的构造析构必须为public，这样也不好，所以最好还是不要加判断了）。

## 保护不常更新的数据结构

> - 解决方式是使用读写锁，读写锁有两种形式：`std::shared_timed_mutex`（C++14），`std::shared_mutex`（C++17），两者几乎没区别。
>
> - 当需要**读数据**时，应该使用读写锁中的`lock_shared()`和`unlock_shared()`，或者使用读锁的RAII管理类模板`shared_lock<>`来封装读写锁对象。
>
>   当需要**写数据**时，和mutex一样，使用`lock()`和`unlock()`，或者使用写锁的RAII管理类模板`lock_guard<>`、`unique_lock<>`来封装读写锁对象。
...
> - 对于`int* a = new int();`而言，`new int()`叫做new表达式，在执行new表达式的时候，会先调用`operator new`分配指定大小的内存，再调用对应的构造函数**（重要！！！）**。
>
...
>
> - **更加完善的回答**：首先需要知道：对于new表达式而言，会先调用`operator new`分配指定大小的内存，再调用对应的构造函数，最后返回构造好的对象的地址并赋值给对应的指针（`int* a = new int();`）。
>
...
> - 其实可以把线程存储期的变量和全局存储期的变量进行对比，会发现其实两者都差不多，只有在存储期上有差别，而在使用时，thread_local也基本上和static是绑定的。
...
- 代码演示：
  ...
  // a, a1是全局变量，对于线程来说，也就是共享数据，以此观测thread_local变量在不同线程，以及同一个线程不同对象中的表现
  A a;
  A a1;
  void f() {	// 分别调用operator()
      a();
  ...
  class A {
  public:
      inline static thread_local int tmp1 = (puts("init"),0);	// 线程静态成员变量
      void operator()() {
          cout << "function" << endl;
  ...
  1. 对于类中的thread_local静态成员变量而言，与thread_local静态全局变量类似，会在线程函数执行前初始化（不论是否创建A的对象，都会如此，也很好理解），但g++有优化：**g++会在首次使用线程全局静态变量的地方按顺序初始化所有的线程全局静态变量，而普通的全局静态变量则和MSVC一样，在主线程函数执行前就初始化**。
     ...
     当然除此之外，我们也可以写一个[弃值表达式](https://www.zhihu.com/question/359039156)来防止Gcc和Clang优化该值：
...
- 线程变量
  ...
  - 示例：
    ...
    - 对于thread_local全局静态变量而言：变量的初始化会**作为线程启动的一部分**，初始化时期早于线程函数运行时期（以上是msvc的实现，对于Gcc而言，需要考虑该值被优化的场景）
...
- 示例：
  ...
  - `shared_future<>`和`future<>`其实没啥区别，他们都是`_State_manager<>`的子类，只是他们之间的构造函数，`get()`函数之间有区别，他们之间最重要的区别（也就是共享和独有的特性），体现在他们通过构造会调用基类的基类（`_Associated_state<>`）中不同的函数（比如共享可以额外调用原子加，原子减）。
...
**任何对非原子（non-atomic）变量的并发访问（其中至少一个是写操作）本身就是未定义行为，除非所有这些并发访问都是通过同一个互斥锁来保护。**

如果两个线程同时访问一个非原子内存位置，且至少一个是写操作，并且他们之间不存在happens-before关系，那么行为是数据竞争，操作结果未定义
...
1. 消息队列改为[#此形式](#线程安全的队列)；同时给模板上SFINAE，只允许特定类型们使用消息队列；并使用CRTP减少冗余代码；并将queue改为循环队列，看看能不能改成无锁队列，使用`shared_ptr<>`包装数据（C++并发编程实战161）
