...
- 根据返回的内容判断返回值用引用/非引用
  ...
  > 返回的内容是对象本身（this指针）
  >
  > - 返回值为引用（需要确保this非临时变量）：使用链式编程。eg：++(++OBJ_NAME)；OBJ_NAME = OBJ_1 = OBJ_2
  > - 返回值为非引用：不使用链式编程
...
public:
	...
	//后置++重载，
	//占位参数只能是int，不能是其他类型
	myinteger operator++(int) {//返回值只能是非引用，因为temp为临时对象，不过在外面可以通过右值引用来延长返回值临时对象的生命周期
		myinteger temp = *this;
		mynum++;
		return temp;
	}
...
// 前置++的返回值为左值引用，其对应的函数调用表达式为左值，可以被const myinteger&接收
// 后置++的返回值为普通类型，其对应的函数调用表达式为右值，可以被const myinteger&接收
...
- 标准转换序列
  ...
  **标准转换序列**是 C++ 编译器为了将一个表达式的类型（源类型）转换为另一个类型（目标类型）而执行的一套预定义的、有序的内置转换规则，一共有三个阶段：
  ...
  - 值类型转换
    ...
    - **左值到右值**: 当在**需要表达式的“值”，而非其“身份”（对象本身）**时才会发生，如下：
