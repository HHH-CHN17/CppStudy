...
>
> - 如果Date既有多参构造函数，又有initializer_list做参数的构造函数，那么在使用列表初始化的时候，**如果传入的参数可以隐式的转换为initializaer_list**，会调用后者，否则调用前者
>
>   ```c++
>   class X {
>   public:
>   	//X(int x){}	// 如果取消改行注释，则A a={1,2};调用A(initializer_list<X> il)
>   };
>   class A {
>   public:
>   	A(int x,int y){cout << "int" << endl;}
>   	A(initializer_list<X> il){cout << "init" << endl;}
>   };
>   
>   int main() {
>   	A a = {1,2};	// 调用A(int,int)
...
> - ` using Base::Base`是**隐式声明继承的**。即假设一个继承构造函数不被相关的代码使用，编译器不会为之产生真正的函数代码，这样比透传基类各种构造函数更加节省目标代码空间。但此时派生类无法对类自身定义的新的类成员进行初始化，通过**就地初始化**解决。
...
- array delete
  ...
  - 如果不用array delete 会导致数组中1-n的对象没有调用析构函数，造成堆内存泄漏，但是栈内存不会泄漏，因为调用delete一定会调用free，调用free
...
- 注意在`GetTemp()`中构造出来的变量会被转换成右值，所以上面的两处拷贝构造其实用移动构造表述更准确
...
不过在C++11中，用于完美转发的函数却不再叫作`move`，而是另 外一个名字`forward`，也是将参数强制转换为右值引用。具体实现如下：
...
- `IamForwording`函数参数必须指定为`T&&`，否则无法接收右值
...
{
	B b;
	cout << hex << &b << endl;		// 000000847333F578：子类对象地址
	cout << hex << &b.vvv << endl;	// 000000847333F578：int变量地址
	cout << hex << &b.vv << endl;	// 000000847333F57C：基类子对象地址
...
[《深入理解C++11》笔记-模板的别名、模板的SFINEA规则_c++ sfinea-CSDN博客](https://blog.csdn.net/WizardtoH/article/details/80907334)

[C++模板进阶指南：SFINAE - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/21314708)
...
- 否则，如果type是一个左值引用，或者该变量被括号包围（`decltype((i))`），那么decltype(type)为T&。
- 否则，decltype(type)为T。
...
int main()
{
    ...
    decltype((isvalue(1))) var15; // const bool,圆括号可以忽略，有问题！！！！去vscode上试试
...
上一篇中说到auto不能带走cv限制符(`const`、`volatile`)，`decltype`是能够带走的。不过，如果创建对象时有`const`或`volatile`限制符，使用`decltype`推导时，其成员不会继承cv限制符。
...
- 注意事项：
  ...
  - 记得加*
    ...
    ```c++
    typedef int(PointArr1)[4];	// 这个不是数组指针，相当于直接定义了一个数组，没有实际意义
...
- 使用typedef 简化指针定义
  ...
  ```c++
  typedef int (*FUNC)(int, int);      //定义一个FUNC代表 int(*)(int, int) 类型
  typedef FUNC(*pfuncAFun)(const char*, FUNC);//定义一个FUNC代表 int(*)(int, int) 类型
...
- 练习题：
  ...
  int (*(*pf())())();
  //	分析的时候由内向外
  //	int (*(*X)())();   // 判断出X是函数指针之后，要把(*X)放在一起看，(*X)为X解引用后对应的函数名
  //	int (*Y)();
  //	简化的时候由外向内
  //	typedef int(*PF_I)();
  //	typedef PF_I(*PF_PFI)();
...
- 练习题：
  ...
  ```c++
  #define OVER0(N) ((N) == 0 ? 1 : (N))
  typedef int(*PFUNC)(int, int);
  typedef PFUNC(*PFUNCARR)[OVER0(5)];
...
- lambda的类型
  ...
  从C++11标准的定义上可以发现，lambda 的类型被定义为“闭包”（closure）的类， 而每个 lambda表达式则会产生一个闭包类型的临时对象（右值)。因此，严格地讲，lambda 函数并非函数指针。不过**C++11标准却允许lambda 表达是向函数指针的转换**，但前提是，lambda 函数没有捕捉任何变量，且函数指针所示的函数原型，必须跟 lambda 函数有着相同的调用方式。
...
- lambda的常量性以及mutable关键字
  ...
  1. 对于常量成员函数而言，不能在函数体内改变class中任何成员变量，所以x不可以更改
