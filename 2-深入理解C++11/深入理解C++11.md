...
- 否则，表达式为**右值**。从概念上（通常也是事实上）右值对应于临时对象，例如从函数返回或通过隐式类型转换创建的对象。大多数 Literals 值（例如 和 ）也是右值
...
3. 由于`_Args`也可以为泛左值或是右值，所以：
   ...
   - `forward<_Ty>(_Arg)`可以将泛左值转发为右值（✔）
     - `forward<_Ty>(_Arg)`可以将右值转发为右值（✔）
   - `forward<_Ty>(_Arg)`可以将右值转发为泛左值（❌）`static_assert()`禁止该转换
  4. 由上，如果直接使用`forward<>()`，那么该模板和`move()`不仅没啥区别，还需要额外指明推导类型，正确用法如下
...
> 总结：`extern "C"` 就是使得被其包裹的函数在编译生成符号表的时候不适用/不使用名字重整技术，所以使用 `extern "C"` 会导致很多问题，比如无法使用函数重载，命名空间，同名，等等特性。（同时也正因为名字重整技术，所以C++的代码生成的ABI无法跨平台，C和C++也无法互相引用对方的ABI，当然C标准也没有规定统一的ABI，）
