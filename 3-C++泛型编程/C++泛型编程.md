...
1. 静态初始化：
   ...
   - **发生时间**: 编译时，就将结果写入可执行文件中，在操作系统读取可执行文件时，就将其内容映射到进程的虚拟内存空间中。
...
- 解释：
  ...
  2. inline函数
     ...
     > 2. 允许同一个函数的定义出现在多个翻译单元之中，并且在链接时只保留其中的一份作为该符号的定义，防止重定义
  ...
  3. inline变量
     ...
     > 1. 允许在多个翻译单元对同一个变量进行定义，并且在链接时只保留其中的一份作为该符号的定义，防止重定义
