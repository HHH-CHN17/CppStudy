...
- 作用一：[声明具有*静态存储期*和*内部链接*的*命名空间成员*（**修饰全局变量/函数，称为静态全局变量/静态函数**）](https://zh.cppreference.com/w/cpp/language/storage_duration)
...
- 作用二：定义具有*静态存储期*且*仅初始化一次*的*块作用域变量*（**修饰局部变量，称为静态局部变量**）
...
- 解析：
  ...
  2. 待决名的意思是在定义的地方，类型还不能决断，需要延后到**模板实参已知时**。而非待决名指类型在定义的地方已经确定。
     ...
     **确定地点：非待决名在定义模板的地方（定义点）确定名字，待决名在生成模板实例的地方（实例化点）确定名字。**
     ...
     - 实例化点：在得知模板实参时，使模板进行模板实例化的地方，发生在语义分析时期，也可以理解成“使用”该模板的地方，或者说调用模板函数，生成类模板对象，生成变量模板实例的地方，需要注意的是类模板实例和类模板中成员函数实例的生成时机是不一样的。
...
struct Y : X<T> {
	void t()const {
		...
		z.f();		// z.f() 是待决名，在定义点和实例化点都会查找该名字的声明，但是显然在这两个阶段都找不到
...
2. 若名称在某种程度上依赖于模板参数，那么就是一个依赖型名称(dependent name，也可以叫待决名)。如果 T 是模板参数，那么`std::vector<T>::iterator` 通常是一个依赖名称，但如果 T 是一个已知的类型别名 (例如使用 T=int时的 T)，那么就是一个非依赖名称。
