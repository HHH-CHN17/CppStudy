...
### 1.4 ID体系：CSID, Stream ID 与 Type ID

RTMP 协议通过三套核心 ID 体系，构建了一个从物理通道到业务逻辑，再到具体数据内容的完整寻址模型。清晰地理解这三者的定义、作用和关系，是掌握 RTMP 的关键。

#### 1.4.1 块流 ID

-   **定义:** Chunk Stream ID，协议**底层**的**物理通道**标识符，用于唯一标识一条**块流 (Chunk Stream)**。
-   **作用:**
    1.  **多路复用:** 实现音、视频和信令等不同块在单一网络连接上的交错传输。
    2.  **维持上下文:** 为头部压缩 (`fmt=1,2,3`) 提供上下文环境。

#### 1.4.2 消息流 ID

-   **定义:** Message Stream ID，应用**上层**的**逻辑通道**标识符，用于唯一标识一条**消息流 (Message Stream)**。
-   **作用:**
    1.  **业务关联:** 将属于同一业务逻辑（如一场直播）的所有消息关联起来。
    2.  **生命周期管理:** 客户端通过 `createStream`, `publish`, `play` 等命令，对此 ID 对应的逻辑流进行创建和操作。

#### 1.4.3 消息类型 ID

-   **定义:** Message Type ID，**消息内容**的**类型**标识符，用于唯一标识一条**消息 (Message)** 的具体用途。
-   **作用:**
    1.  **内容解析:** 告知接收端应如何解析消息负载 (Message Payload) 中的数据。
    2.  **协议调度:** 服务器和客户端根据此 ID，对不同类型的消息采取不同的处理逻辑。
...
| **核心作用** | 块的多路复用，<br>维持头部压缩上下文 | 关联一组有业务逻辑的消息    | 定义消息负载的解析方式    |
| **生命周期** | 与 TCP 连接共存                      | 可按需创建和销毁            | 单次消息有效              |
