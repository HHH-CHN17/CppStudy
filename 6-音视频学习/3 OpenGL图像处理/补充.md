...
struct GLTex{
    ...
    atomic_bool is_using = false;// 当渲染线程往该texture中写入了数据，且主线程没处理该texture时，为true
    GLTex* next = nullptr;
}	// 这个结构体可以尝试用位域优化空间
```

首先在initFBO的时候，我们先创建一个长度为n+1的链表，以及n个Texid，并赋值给

在这个环形链表中，头节点存放了渲染时间最早的texture，尾节点存放了渲染时间最晚的texture（可以理解成一个队列），
